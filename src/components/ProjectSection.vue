    ...
    const Projects = ref([
        {
            ...
            image: require('./src/assets/Vue+Tailwind.jpg'),
...
<template>
    <section class="text-white mt-20" id="projects">
        <div class="px-4 xl:pl-16">
            <h2 class="text-4xl font-bold text-white mb-8 md:text-center md:mt-0 mt-8">
                My 
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#93FB9D] to-[#09C7FB]">
                    Projects
                </span>
            </h2>
            <ul class="px-4 sm:py-16 xl:pr-16 grid grid-cols-1 gap-6 pt-10 sm:grid-cols-2 md:gap-10 md:pt-12 lg:grid-cols-2" data-aos="fade-right">
                <div v-for="project in filteredProjects" :key="project.id">
                    <div class="h-52 md:h-[35rem] rounded-t-xl relative group" 
                    :style="{ backgroundImage: 'url(' + project.image + ')', backgroundSize: 'cover'}">
                        ...
                        hidden group-hover:flex group-hover:bg-opacity-80 transition-all delay-500">
                    ...
                    <div class="text-white rounded-b-xl mt-3 bg-[#111A3E] shadow-lg border border-[#1F1641] py-6 px-4">
                        <h3 class="text-lg font-semibold uppercase lg:text-xl text-transparent bg-clip-text bg-gradient-to-r from-[#93FB9D] to-[#09C7FB]"> {{ project.title }} </h3>
